...
class GPT2PPL:
    ...
    def getPPL(self,sentence):
        ...
        # 调试: 检查 nlls 列表中的 NaN 值
        for i, nll in enumerate(nlls):
          if torch.isnan(nll):
            print(f"NaN found in nlls at index {i}")

        if any(torch.isnan(nll) for nll in nlls):
          raise ValueError("NaN found in nlls")

        ppl = torch.exp(torch.stack(nlls).sum() / end_loc).item()
        #if torch.isnan(ppl):
         # raise ValueError("NaN found in perplexity calculation")
